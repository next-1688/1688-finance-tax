...
from _const import GATEWAY_TOOL_CALC_VAT, TOOL_CODE_CALC_VAT
...
def calculate_vat_only(
    ...
    upstream_invoice_type: Optional[str] = None,
) -> dict:
    """
    商品含税定价（仅增值税）
    ...
    body = {
        "scene": "TAX_INCLUDED_PRICE_ONLY_VAT" if taxpayer_type == "SMALL_SCALE_TAXPAYER" else None,
    ...
    if upstream_invoice_type:
        body["upstreamInvoiceType"] = upstream_invoice_type

    payload = merge_gateway_skill_fields(body, TOOL_CODE_CALC_VAT)
    data = api_post(GATEWAY_TOOL_CALC_VAT, body=payload)
